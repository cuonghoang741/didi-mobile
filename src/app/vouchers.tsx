...
const VouchersScreen = () => {
  ...
  const fetchVouchers = useCallback(async () => {
    ...
    try {
      ...
      console.error('[VouchersScreen] Error fetching vouchers:', error);
    } finally {
      setIsLoading(false);
  ...
  const filteredVouchers = vouchers.filter((cv) => {
    const now = new Date();
    const validUntil = new Date(cv.voucher.valid_until);
    const isExpired = validUntil < now;
    const isUsed = cv.is_used;

    if (activeTab === 'unused') {
      return !isUsed && !isExpired && cv.voucher.is_active;
    } else {
      return isUsed || isExpired || !cv.voucher.is_active;
    }
  });

  const unusedCount = vouchers.filter((cv) => {
    const now = new Date();
    const validUntil = new Date(cv.voucher.valid_until);
    return !cv.is_used && validUntil >= now && cv.voucher.is_active;
  }).length;
  ...
  const handleUseVoucher = (voucher: CustomerVoucher) => {
    // Navigate to checkout or apply voucher
    router.push('/cart' as any);
  };

  const handleViewVoucher = (voucher: CustomerVoucher) => {
    // Show voucher details modal or navigate to detail page
    // For now, just a placeholder
  };

  const renderVoucherCard = (cv: CustomerVoucher) => {
    const now = new Date();
    const validUntil = new Date(cv.voucher.valid_until);
    const isExpired = validUntil < now;
    const isUsed = cv.is_used;
    const isInactive = !cv.voucher.is_active;

    const isDisabled = isUsed || isExpired || isInactive;

    return (
      <View key={cv.id} style={[styles.voucherCard, isDisabled && styles.voucherCardDisabled]}>
        <View style={styles.voucherContent}>
          {/* Voucher Icon */}
          {getVoucherIcon(cv.voucher)}
          ...
          <View style={styles.voucherInfo}>
            ...
            {cv.voucher.description && (
              <Typography
                variant='text'
                size='sm'
                style={[styles.voucherDescription, isDisabled && styles.textDisabled]}
              >
                {cv.voucher.description}
              </Typography>
            )}
        ...
        <View style={styles.voucherFooter}>
          <View style={styles.dateContainer}>
            <Feather
              name='clock'
              size={16}
              color={isDisabled ? theme.colors.text.tertiary : '#16A34A'}
            />
            <Typography
              variant='text'
              size='sm'
            ...
            >
              {isUsed
                ? `${t('vouchers.usedOn')} ${formatDate(cv.used_at || cv.voucher.valid_until)}`
                : isExpired
                  ? `${t('vouchers.expiredOn')} ${formatDate(cv.voucher.valid_until)}`
                  : `${t('vouchers.validUntil')} ${formatDate(cv.voucher.valid_until)}`}
            </Typography>
          </View>

          <Pressable onPress={() => (isDisabled ? handleViewVoucher(cv) : handleUseVoucher(cv))}>
            <Typography
              variant='text'
              size='sm'
              weight='semiBold'
              style={[styles.actionText, { color: isDisabled ? '#3B82F6' : '#16A34A' }]}
            >
              {isDisabled ? t('vouchers.viewVoucher') : t('vouchers.useNow')}
            </Typography>
          </Pressable>
        </View>
      </View>
        ...
        <View style={styles.tabsContainer}>
          ...
          >
            <Typography
              variant='text'
              size='md'
              weight={activeTab === 'unused' ? 'semiBold' : 'regular'}
              style={[styles.tabText, activeTab === 'unused' && styles.tabTextActive]}
            >
              {t('vouchers.unused')}
            </Typography>
            {unusedCount > 0 && (
          ...
          >
            <Typography
              variant='text'
              size='md'
              weight={activeTab === 'used' ? 'semiBold' : 'regular'}
              style={[styles.tabText, activeTab === 'used' && styles.tabTextActive]}
            >
              {t('vouchers.usedExpired')}
            </Typography>
          </Pressable>
        </View>
            ...
            {filteredVouchers.length === 0 ? (
              <View style={styles.emptyContainer}>
                ...
                <Typography variant='text' size='md' style={styles.emptyText}>
                  {activeTab === 'unused'
                    ? t('vouchers.noUnusedVouchers')
                    : t('vouchers.noUsedVouchers')}
                </Typography>
              </View>
            ) : (
