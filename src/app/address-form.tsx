...
const AddressFormScreen = () => {
  ...
  const lookupAddressFromPostalCode = useCallback(
  ...
  const handlePickImage = async () => {
    try {
      ...
      if (!permissionResult.granted) {
        Alert.alert(t('common.error'), t('addresses.errors.permissionDenied'));
        return;
      }
  ...
  const showImageOptions = () => {
    Alert.alert(t('addresses.form.addPhoto'), t('addresses.form.selectPhotoSource'), [
      {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing && id) {
        const { error } = await db.from('customer_addresses').update(addressData).eq('id', id);

        if (error) throw error;
      } else {
        const { error } = await db.from('customer_addresses').insert(addressData);
          ...
          >
            ...
            <View style={styles.inputGroup}>
              <Typography variant='text' size='sm' weight='medium' style={styles.label}>
                {t('addresses.form.nickname')}
                <Typography style={styles.required}> *</Typography>
              </Typography>
              <TextInput
            ...
            <View style={styles.imageSection}>
              <Typography variant='text' size='sm' weight='bold' style={styles.imageSectionTitle}>
                {t('addresses.form.addressPhotoKanji')}
                <Typography style={styles.required}> *</Typography>
              </Typography>
            ...
            >
              <View style={[styles.checkbox, formData.is_default && styles.checkboxChecked]}>
                {formData.is_default && <Feather name='check' size={14} color='#FFFFFF' />}
              </View>
