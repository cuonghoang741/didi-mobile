...
const BankTransferConfirmScreen = () => {
  ...
  useEffect(() => {
    if (settings?.bank_accounts) {
      try {
        const accounts =
          typeof settings.bank_accounts === 'string'
  ...
  const handleSkipProof = () => {
    setShowProofModal(false);
    Alert.alert(t('common.notice'), t('bankTransfer.pleaseTransferAndUpload'));
  };
        ...
        {bankAccounts.length > 1 && (
          <View style={styles.tabContainer}>
            {bankAccounts.map((account, index) => (
              <Pressable
                key={account.id}
                style={[styles.tab, selectedAccountIndex === index && styles.tabActive]}
              ...
              >
                ...
                <Typography
                  ...
                  style={[styles.tabText, selectedAccountIndex === index && styles.tabTextActive]}
                  numberOfLines={1}
                >
                  {account.bank_name}
                </Typography>
        ...
        {selectedAccount && (
          <View style={styles.bankCardContainer}>
            <View style={styles.bankCard}>
              <LinearGradient
                colors={
                  selectedAccountIndex === 0 ? ['#4A6FA5', '#1E3A5F'] : ['#2D8B74', '#1A5C4E']
                }
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
        ...
        <View style={styles.detailsCard}>
          ...
          <View style={styles.detailRow}>
            ...
            <View style={styles.copyRow}>
              ...
              <Pressable
                onPress={() =>
                  handleCopy(selectedAccount?.account_number || '', t('checkout.accountNumber'))
                }
                hitSlop={8}
              >
